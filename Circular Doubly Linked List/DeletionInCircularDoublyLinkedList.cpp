#include <iostream>
using namespace std;

class DoublyLinkedList
{
public:
    int data;
    DoublyLinkedList *next;
    DoublyLinkedList *previous;

    DoublyLinkedList(int data)
    {
        this->data = data;
        this->next = NULL;
        this->previous = NULL;
    }
    ~DoublyLinkedList()
    {
        cout << "Deleted " << this->data << endl;
        if (this->next == NULL)
        {
            delete this;
        }
    }
};

void TraversalDoublyLinkedList(DoublyLinkedList *&node)
{
    DoublyLinkedList *start = node;
    cout << start->data << endl;
    start = start->next;
    while (node != start)
    {
        cout << start->data << endl;
        start = start->next;
    }
}

void InsertionAtDoublyLinkedList(DoublyLinkedList *&node, int data, int positionData)
{
    DoublyLinkedList *temp = node;
    while (temp->data != positionData)
    {
        temp = temp->next;
    }
    DoublyLinkedList *nextNode = temp->next;
    DoublyLinkedList *newNode = new DoublyLinkedList(data);
    newNode->previous = temp;
    temp->next = newNode;

    newNode->next = nextNode;
    nextNode->previous = newNode;
}

void DeletionAtDoublyCircularLinkedList(DoublyLinkedList *&node, int deleteTarget)
{
    DoublyLinkedList *temp = node;
    while (temp->data != deleteTarget)
    {
        temp = temp->next;
    }
    DoublyLinkedList *previousNode = temp->previous;
    DoublyLinkedList *nextNode = temp->next;

    previousNode->next = nextNode;
    nextNode->previous = previousNode;

    delete temp;
}
int main()
{
    DoublyLinkedList *node1 = new DoublyLinkedList(1);
    DoublyLinkedList *node2 = new DoublyLinkedList(2);
    DoublyLinkedList *node3 = new DoublyLinkedList(3);
    DoublyLinkedList *node4 = new DoublyLinkedList(4);

    node1->previous = node4;
    node1->next = node2;

    node2->previous = node1;
    node2->next = node3;

    node3->previous = node2;
    node3->next = node4;

    node4->previous = node3;
    node4->next = node1;

    TraversalDoublyLinkedList(node1);
    InsertionAtDoublyLinkedList(node1, 0, 3);
    TraversalDoublyLinkedList(node1);

    DeletionAtDoublyCircularLinkedList(node1, 0);
    TraversalDoublyLinkedList(node1);

    return 0;
}